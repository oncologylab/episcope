...
test_that("significant priority pathways bypass per-topic plot ranking", {
  out_dir <- file.path(
    tempdir(),
    paste0("topic-pathway-priority-", sample.int(1e8, 1L))
  )
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
  topic_terms <- data.table::data.table(
    topic = c("Topic1", "Topic1", "Topic1"),
  ...
  with_mocked_bindings(
    plot_topic_pathway_enrichment_heatmap(
      ...
      min_genes = 2L,
      top_n_per_topic = Inf,
      dot_top_n_per_topic = 1L,
      dot_priority_patterns = "Epithelial Mesenchymal",
      max_pathways = Inf,
      make_heatmap = FALSE,
      make_dotplot = TRUE,
      pathway_backend = "enrichly"
  ...
  expect_setequal(
    plotted$pathway,
    c(
      "Toy_DB: Top generic pathway",
      "Toy_DB: Epithelial Mesenchymal Transition"
    )
  )
})
