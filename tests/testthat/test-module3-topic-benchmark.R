...
test_that("Module 3 condition-pair report uses schema 10 pathway and GRN modes", {
  ...
  expect_match(html, "attach=clamp(halfHeight*.45,6,9)", fixed = TRUE)
  expect_match(html, "radius=clamp(13+node.r*.22,15,20)", fixed = TRUE)
  expect_match(html, "stem=clamp(radius*.2,3,5)", fixed = TRUE)
  ...
  expect_match(html, "'stroke-width':Math.max(6.5,width+4.4)", fixed = TRUE)
