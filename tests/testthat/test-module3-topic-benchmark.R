...
test_that("Module 3 condition-pair report uses schema 10 pathway and GRN modes", {
  ...
  expect_match(html, "rowH=Math.min(27", fixed = TRUE)
  ...
  expect_match(html, "fitSvgText(t,maxWidth,14,14)", fixed = TRUE)
  ...
  expect_match(html, '<option value="filter" selected>Pathway only</option>', fixed = TRUE)
  expect_match(html, '<option value="highlight">Full topic + highlight</option>', fixed = TRUE)
  ...
  expect_match(html, "function robustColorLimit", fixed = TRUE)
  ...
  expect_match(html, "centerX=node.x+corner[0]*(halfWidth+stem+radius*.5)", fixed = TRUE)
  ...
  expect_match(html, "function fitSvgText(node,maxWidth,minSize=10,maxSize=Infinity)", fixed = TRUE)
  expect_match(html, "Math.ceil(mdsTextWidth(q.text,q.fontSize)*.88)", fixed = TRUE)
  expect_match(html, "q.h=compact?25:36", fixed = TRUE)
  expect_match(html, "q.maxLeader=compact?100:160", fixed = TRUE)
  expect_match(html, "pointClearance=36", fixed = TRUE)
  expect_match(html, "q.layoutHash=Array.from(String(q.id))", fixed = TRUE)
  expect_match(html, "const conflicted=items.filter", fixed = TRUE)
  expect_match(html, "const others=items.filter(other=>other!==q),clean=candidatesFor(q)", fixed = TRUE)
  expect_match(html, "fitSvgText(t,maxWidth,14,14)", fixed = TRUE)
  expect_match(html, "networkSelfLoopArrow", fixed = TRUE)
  expect_match(html, "attach=clamp(halfHeight*.25,3,4)", fixed = TRUE)
  expect_match(html, "radius=clamp(6+node.r*.06,7,9)", fixed = TRUE)
  expect_match(html, "stem=clamp(radius*.14,1.2,2)", fixed = TRUE)
  expect_match(html, "' L '+loop.arcStartX+' '+loop.arcStartY", fixed = TRUE)
  expect_match(html, "' A '+loop.radius+' '+loop.radius+' 0 1 '+loop.sweep", fixed = TRUE)
  expect_match(html, "' L '+loop.endX+' '+loop.endY", fixed = TRUE)
  expect_match(html, "Number(a.edge.from===a.edge.to)-Number(b.edge.from===b.edge.to)", fixed = TRUE)
  expect_match(html, "selfLoop?Math.max(2.1,width):width", fixed = TRUE)
  expect_match(html, "class:'selfLoopHalo'", fixed = TRUE)
  expect_match(html, "'stroke-width':Math.max(4.2,width+2.2)", fixed = TRUE)
  expect_match(html, "selfLoop?'url(#networkSelfLoopArrow)':'url(#networkArrow)'", fixed = TRUE)
  expect_match(html, ".selfLoopHalo{fill:none;stroke:#fff", fixed = TRUE)
  expect_match(html, "assignNetworkSelfLoopCorners(networkState)", fixed = TRUE)
