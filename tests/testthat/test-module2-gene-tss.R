...
test_that("Module 2 can resolve gene TSS from project config", {
  ...
  res <- link_tf_targets(compact, pred, project_config = list(ref_genome = "hg38", gene_tss = tss_path, module2 = list(threshold_tf_target_corr_r = 0.8, threshold_fp_target_corr_r = 0.8)), max_distance_bp = 1000, n_cores = 1, verbose = FALSE)
