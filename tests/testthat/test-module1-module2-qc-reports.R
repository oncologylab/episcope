test_that("Module 1 QC report writes an HTML summary", {
  ...
  expect_true(grepl("Run configuration", page, fixed = TRUE))
  ...
  expect_true(grepl("FP score distribution summary", page, fixed = TRUE))
  expect_true(grepl("Gene expression distribution", page, fixed = TRUE))
  expect_true(grepl("RNA PCA", page, fixed = TRUE))
  expect_true(grepl("ATAC PCA", page, fixed = TRUE))
  expect_true(grepl("Canonical-filtered footprints per condition", page, fixed = TRUE))
  expect_true(grepl("Active predicted TFBS per condition", page, fixed = TRUE))
  ...
  expect_true(grepl("6. Top Predicted Bound TF per Motif", page, fixed = TRUE))
  ...
  expect_true(grepl("Module 1 processing funnel", page, fixed = TRUE))
  expect_true(grepl("What Happened", page, fixed = TRUE))
  ...
  expect_true(file.exists(file.path(out_dir, "reports", "module1_tfbs_explorer.html")))
