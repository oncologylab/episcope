...
fp_manifest <- load_footprints(
  root_dir   = "/data/homes/cy232/cutntag/humanPDAC/Nutrients_basal/TOBIAS_merged_peaks",
  db_name    = "JASPAR2024",
  out_dir    = "/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024",
  n_workers  = 20,
  # n_samples  = 2,
...
readr::write_csv(fp_manifest, "/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024_manifest.csv")
fp_manifest <- readr::read_csv("/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024_manifest.csv")
...
readr::write_csv(fp_aligned$fp_bound, paste0("/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024/fp_bounds_", db, ".csv"))
readr::write_csv(fp_aligned$fp_score, paste0("/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024/fp_scores_", db, ".csv"))
readr::write_csv(fp_aligned$fp_annotation, paste0("/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024/fp_annotation_", db, ".csv"))
...
readr::write_csv(fp_aligned_normalized$fp_score, paste0("/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024/fp_scores_qn_", db, ".csv"))

fp_aligned_normalized_manifest <- save_footprints(fp_aligned_normalized, out_dir  = paste0("/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024/fp_aligned_normalized_", db))
readr::write_csv(fp_aligned_normalized_manifest, paste0("/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024/fp_aligned_normalized_manifest_", db, ".csv"))


sample_metadata <- readxl::read_excel("/data/homes/yl814/episcope_test/nutrient_stress/sample_metadata.xlsx", na = "NA")
...
tf_list  <- motif_db |> tidyr::separate_rows(HGNC, sep = "::") |> dplyr::filter(!is.na(HGNC), HGNC != "") |> dplyr::distinct(HGNC) |> dplyr::pull(HGNC)
...
fp_aligned_normalized_filtered_manifest <- filter_footprints(
  ...
  out_dir             = paste0("/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024/fp_aligned_normalized_filtered_", db),
...
readr::write_csv(fp_aligned_normalized_filtered_manifest, paste0("/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024/fp_aligned_normalized_filtered_manifest_", db, ".csv"))
...
fp_corr_manifest <- tf_corr_footprints(
  ...
  out_dir     = paste0("/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024/fp_aligned_normalized_filtered_corr_", db),
  build_args  = strict_build_args,
  n_workers   = 15,
  cor_method  = "pearson",
  min_non_na  = 5L
...
readr::write_csv(fp_score, paste0("/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024/fp_score_strict_tf_filtered_corr_", db, ".csv"))
readr::write_csv(fp_bound, paste0("/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024/fp_bound_strict_tf_filtered_corr_", db, ".csv"))
readr::write_csv(fp_annotation, paste0("/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024/fp_annotation_strict_tf_filtered_corr_", db, ".csv"))

unique(fp_annotation$fp_peak)

# Strict
strict_set <- build_grn_set(
  fp_score      = fp_score,
  fp_bound      = fp_bound,
  fp_annotation = fp_annotation,
  atac_score    = atac_score,
  atac_overlap  = atac_overlap,
  rna           = strict_rna,          # strict matched RNA subset
  metadata      = stric_metadata,      # strict metadata table
  tf_list       = tf_list,
  motif_db      = motif_db,
  label_col     = "strict_match_rna",
  expected_n    = 23
)
strict_set <- filter_by_min_bound(strict_set, min_bound = 1L)
...
#   peaks      = strict_set$atac_score,
#   genes      = unique(c(strict_set$rna$HGNC, strict_set$rna$ensembl_gene_id)),
#   flank_bp   = 30000,
#   mode       = "TSS",
...
atac_res <- correlate_atac_to_genes(grn_set = strict_set, gh_tbl = gh_std, gene_mode = "both", fdr = threshold_atac_gene_corr_p, r_abs_min = threshold_atac_gene_corr_abs_r)

# shrink to only correlated peaks
strict_set_filtered <- filter_grn_by_corr(strict_set, atac_res$atac_gene_corr_kept)

# Run fp gene correlations (adjusted p-value < 0.05, |r| >= 0.3)
fp_res <- correlate_fp_to_genes(
  grn_set              = strict_set_filtered,
...
base_dir <- "/data/homes/yl814/episcope_test/nutrient_stress/fp_jaspar2024"
match_mode <- "strict"  # strict lenient
regulated_genes <- 1.5  # 1.5 2
delta_link <- 1         # 2 1
regulation_priors <- "genehancer" # "genehancer"
lighting_folder <- file.path(base_dir, paste("lighting", regulation_priors, match_mode, db, "regulated_genes", regulated_genes, "delta_link", delta_link, sep = "_"))
...
basal <- make_basal_links(
  ...
  fp_annotation     = strict_set$fp_annotation,
...
episcope::render_link_network_delta_topic_simple(
  ...
  out_html = file.path(dirname(comp_csv),
                       sprintf("%s_topic-%d_subnetwork_delta.html",
...
render_tf_hub_delta_network(
  comp_csv  = file.path(
    lighting_folder,
    "AsPC1_10_Gln.Arg_vs_AsPC1_10_FBS_delta_links_filtered_lda_K20.csv"),
  input_tf  = "HNF4A",
  edge_filter_min = 1,
