...
## ------------------------------------------------------------------
## 1) Basic TF → cell / lineage mapping
## ------------------------------------------------------------------
...
## ------------------------------------------------------------------
## 3) Core helpers: RNA mapping / correlation
## ------------------------------------------------------------------
...
build_ko_summary_table_for_tf <- function(tf,
  ...
  if (!all(c("r_fp", "p_adj_fp") %in% names(tf_links_mode))) {
    stop("tf_gene_links must have columns 'r_fp' and 'p_adj_fp'.")
  }

  fp_sig_genes <- tf_links_mode %>%
    dplyr::filter(
      !is.na(r_fp),
      !is.na(p_adj_fp),
      r_fp > r_cut_main,
  ...
  ## Base universes for each method -----------------------------------
  ...
  get_fp_sets <- function(base_genes, p_cut) {
    ...
    pass_rows <- !is.na(links$r_fp) &
      !is.na(links$p_adj_fp) &
      links$r_fp > r_cut_main &
      links$p_adj_fp < p_cut
  ...
  get_rna_sets <- function(base_genes, p_cut) {
    ...
    g_sub <- rna_corr_full[rna_corr_full$gene_key %in% base_genes,
                           , drop = FALSE]
    if (!nrow(g_sub)) {
      return(list(passing = character(0), non_passing = character(0)))
    }
  ...
  method_defs <- tibble::tibble(
    ...
    method_lab  = factor(
      c(
        ...
        "RNA (filtered by GeneHancer links)",
...
plot_ko_summary_violin_for_tf <- function(tf,
  ...
  ## Row 4: percent of log2FC bins, split by group --------------------
  df_bins <- df %>%
    dplyr::mutate(
  ...
  outfile <- file.path(
    out_dir,
    sprintf(
      "%s_%s_%s_%s_%s_FP_RNA_corr_R%.1f_summary_violin_plots.pdf",
      tf,
      ko_type,
      tf_cell,
      tf_line,
...
## ------------------------------------------------------------------
## 6) KO-table helper + example usage for all TFs
## ------------------------------------------------------------------
...
## SOX9 -----------------------------------------------------------------
if (exists("SOX9_KO")) {
  ko_tbl_SOX9 <- make_ko_tbl(SOX9_KO, "SOX9")
...
## KLF5 -----------------------------------------------------------------
if (exists("KLF5_KO")) {
  ko_tbl_KLF5 <- make_ko_tbl(KLF5_KO, "KLF5")
...
## FOXA2 ----------------------------------------------------------------
if (exists("FOXA2_KO")) {
  ko_tbl_FOXA2 <- make_ko_tbl(FOXA2_KO, "FOXA2")
