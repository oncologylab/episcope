...
.m3cr_pathway_expression_scores <- function(topic_pathways,
  ...
  modeled_genes <- if (!is.null(gene_topics) && nrow(gene_topics)) {
    unique(toupper(as.character(data.table::as.data.table(gene_topics)$gene_key)))
  } else {
    unique(toupper(as.character(expression$gene_key)))
  }
  expression[, `:=`(
    condition_id = as.character(condition_id),
    gene_key = as.character(gene_key),
  ...
  expression <- expression[
    gene_match %in% modeled_genes & is.finite(gene_expr),
    .(gene_expr = max(gene_expr, na.rm = TRUE)),
