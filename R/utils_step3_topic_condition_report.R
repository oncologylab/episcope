...
function drawMds(){
  ...
  rows.forEach(d=>{
    const id=String(d.comparison_label||d.group_label),x=sx(num(d.MDS1)),y=sy(num(d.MDS2)),
      ...
      raw=String(d.mds_label||d.display_label||id).replace(/_/g,' ').trim(),
      text=labelMap.get(raw)||raw,
...
function selectedTfTargets(cond,tf){const indexed=(TF_TARGETS_BY_COND.get(String(cond))||new Map()).get(tfKey(tf));return indexed?new Set(indexed):new Set(edgeRowsForTf(cond,tf).map(d=>String(d.gene_key)))}
