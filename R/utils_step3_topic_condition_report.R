...
function mdsCandidateLeader(q,candidate){const bx=clamp(q.pointX,candidate.x,candidate.x+candidate.w),by=clamp(q.pointY,candidate.y,candidate.y+candidate.h),dx=bx-q.pointX,dy=by-q.pointY,len=Math.max(1,Math.hypot(dx,dy)),r=q.selected?30:24;return{x1:q.pointX+dx/len*r,y1:q.pointY+dy/len*r,x2:bx,y2:by}}
...
function mdsVisibleLeaders(candidates){const chosen=[];candidates.filter(d=>d.length>=12).sort((a,b)=>Number(b.selected)-Number(a.selected)||a.length-b.length||a.id.localeCompare(b.id)).forEach(d=>{if(!chosen.some(x=>mdsSegmentDistance(d.segment,x.segment)<5))chosen.push(d)});return chosen}
...
function mdsPruneLeaderLines(){const nodes=Array.from(byId('mdsLayer').querySelectorAll('line')).filter(x=>x.getAttribute('stroke')==='#59635e'),candidates=nodes.map((node,i)=>{const segment={x1:num(node.getAttribute('x1')),y1:num(node.getAttribute('y1')),x2:num(node.getAttribute('x2')),y2:num(node.getAttribute('y2'))},d=GROUP_MDS[i]||{},id=String(d.comparison_label||d.group_label||i);return{node,segment,id,selected:id===cond1Select.value||id===cond2Select.value,length:Math.hypot(segment.x2-segment.x1,segment.y2-segment.y1)}}),visible=new Set(mdsVisibleLeaders(candidates).map(d=>d.node));candidates.forEach(d=>{if(!visible.has(d.node))d.node.remove()})}
