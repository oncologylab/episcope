...
.m3cr_network_panel_html <- function() {
  ...
  paste0(
    ...
    "<label id=\"networkPrimaryOnlyLabel\" title=\"Keep each TF only in its single model-wide primary topic, defined by the highest mean raw theta across all available condition::TF documents.\">Primary topic only <input id=\"networkPrimaryOnly\" type=\"checkbox\"/></label>",
...
const CONDITION_DOCUMENT_MODE=String(REPORT_STATE.document_unit||'condition_tf')==='condition';
...
function indexTfTopics(){TF_TOPIC_INDEX=new Map();TF_GLOBAL_TOPIC_INDEX=new Map();TF_GLOBAL_PRIMARY_TOPIC=new Map();TF_BY_CONDITION=new Map();TF_LABELS=new Map();const cols=TF_TOPIC&&TF_TOPIC.columns||[],data=TF_TOPIC&&TF_TOPIC.data||[],at=name=>data[cols.indexOf(name)]||[],condition=at('comparison_label').length?at('comparison_label'):at('group_label'),tf=at('tf_upper').length?at('tf_upper'):at('tf'),display=at('tf_display').length?at('tf_display'):at('tf'),topic=at('topic_num').length?at('topic_num'):at('topic'),theta=at('theta'),primaryTopic=at('primary_topic_num'),hasCanonicalPrimary=cols.includes('primary_topic_num'),n=theta.length;for(let i=0;i<n;i++){const c=String(condition[i]||''),key=tfKey(tf[i]),t=topicNum(topic[i]),v=num(theta[i],NaN),p=topicNum(primaryTopic[i]);if(!c||!key||!Number.isFinite(t)||!Number.isFinite(v))continue;const idx=c+'\t'+key+'\t'+t,old=TF_TOPIC_INDEX.get(idx);if(old===undefined||v>old)TF_TOPIC_INDEX.set(idx,v);if(hasCanonicalPrimary&&Number.isFinite(p))TF_GLOBAL_PRIMARY_TOPIC.set(key,p);if(!TF_BY_CONDITION.has(c))TF_BY_CONDITION.set(c,new Set());TF_BY_CONDITION.get(c).add(key);if(!TF_LABELS.has(key))TF_LABELS.set(key,String(display[i]||tf[i]||key))}const aggregate=new Map();TF_TOPIC_INDEX.forEach((value,index)=>{const parts=index.split('\t'),key=parts[1],topic=Number(parts[2]),aggregateKey=key+'\t'+topic,record=aggregate.get(aggregateKey)||{sum:0,n:0};record.sum+=value;record.n++;aggregate.set(aggregateKey,record)});aggregate.forEach((record,key)=>TF_GLOBAL_TOPIC_INDEX.set(key,record.sum/record.n));if(!hasCanonicalPrimary){const bestScores=new Map();TF_GLOBAL_TOPIC_INDEX.forEach((value,index)=>{const parts=index.split('\t'),key=parts[0],topic=Number(parts[1]),best=bestScores.get(key),primary=TF_GLOBAL_PRIMARY_TOPIC.get(key);if(best===undefined||value>best||(value===best&&topic<primary)){bestScores.set(key,value);TF_GLOBAL_PRIMARY_TOPIC.set(key,topic)}})}}
...
function groupTheta(cond,topic){const direct=()=>{const r=GROUP_TOPIC.find(d=>String(d.comparison_label||d.group_label)===String(cond)&&topicNum(d.topic_num||d.topic)===Number(topic));return r?num(r.theta_mean):0};if(CONDITION_DOCUMENT_MODE)return direct();const mode=byId('thetaAggregation')?byId('thetaAggregation').value:'all';if(mode==='matched'&&cond2Select.value){const keys=matchedTfKeys();if(keys.length)return keys.reduce((s,tf)=>s+tfTheta(cond,tf,topic),0)/keys.length}return direct()}
...
function conditionTopicMetricLabel(){if(conditionTopicMetric&&conditionTopicMetric.value==='rna_delta')return cond2Select.value?'share of positive pairwise RNA activity':'assigned-gene RNA activity share';if(CONDITION_DOCUMENT_MODE)return'P(topic | condition)';const matched=byId('thetaAggregation')&&byId('thetaAggregation').value==='matched'&&cond2Select.value;return matched?'mean theta across matched TF documents':'mean theta across available TF documents'}
function tfTopicMetricLabel(){return CONDITION_DOCUMENT_MODE?'TF-topic evidence probability':'P(topic | condition::TF)'}
function tfTheta(cond,tf,topic){return num(TF_TOPIC_INDEX.get(String(cond)+'\t'+tfKey(tf)+'\t'+Number(topic)),0)}
function tfOverallTheta(tf,topic){return num(TF_GLOBAL_TOPIC_INDEX.get(tfKey(tf)+'\t'+Number(topic)),0)}
function tfOverallPrimaryTopic(tf){const topic=TF_GLOBAL_PRIMARY_TOPIC.get(tfKey(tf));return Number.isFinite(topic)?topic:NaN}
function selectTopTfTopic(tf){if(!tf)return;const best=tfOverallPrimaryTopic(tf);if(Number.isFinite(best)&&Array.from(topicSelect.options).some(option=>Number(option.value)===best))topicSelect.value=String(best);pathwaySelect.value=''}
...
function updateProbabilityPanelTitles(){const label=tfSelect.value?' - '+tfLabel():'',rna=conditionTopicMetric&&conditionTopicMetric.value==='rna_delta',aggregation=byId('thetaAggregation'),matched=!CONDITION_DOCUMENT_MODE&&aggregation&&aggregation.value==='matched'&&cond2Select.value,n=matched?matchedTfKeys().length:0;byId('conditionProbabilityTitle').textContent=rna?'Differential RNA Activity':'Topic Activity';byId('conditionProbabilityTitle').title=rna?'The same expressed assigned genes are compared in both conditions; positive log2 expression differences are summed and normalized by topic.':CONDITION_DOCUMENT_MODE?'Direct fitted probability P(topic | condition).':matched?'Equal-weight mean theta over the same '+n+' TF documents in both selected conditions.':'Equal-weight mean theta over every available condition::TF document in each condition.';byId('tfConditionProbabilityTitle').textContent='TF Probability'+label;byId('tfConditionProbabilityTitle').title=CONDITION_DOCUMENT_MODE?(tfSelect.value?'Post-model TF-topic evidence for '+tfLabel():'Post-model TF-topic evidence for the selected topic.'):(tfSelect.value?'Condition::TF topic probabilities for '+tfLabel():'TF probabilities for the selected topic')}
function refresh(){['mdsTooltip','activityTooltip','butterflyTooltip','tfButterflyTooltip','pathTooltip','networkTooltip'].forEach(hideTip);ensureConditionColors();const overall=overallMode();if(overall){cond2Select.value='';tfSelect.value='';pathwayDeOnly.checked=false;if(networkOpen)closeNetwork()}else if(cond2Select.value===cond1Select.value)cond2Select.value='';const paired=!!cond2Select.value,aggregation=byId('thetaAggregation'),aggregationLabel=byId('thetaAggregationLabel'),primaryLabel=byId('networkPrimaryOnlyLabel'),topicCutoff=byId('networkThetaCutoff'),topicPreset=byId('networkThetaPreset');cond2Select.disabled=overall;cond1Color.disabled=overall;cond2Color.disabled=overall||!paired;if(aggregation)aggregation.disabled=overall||CONDITION_DOCUMENT_MODE;if(aggregationLabel){aggregationLabel.hidden=CONDITION_DOCUMENT_MODE;aggregationLabel.style.display=CONDITION_DOCUMENT_MODE?'none':''}if(primaryLabel&&CONDITION_DOCUMENT_MODE)primaryLabel.title='Keep each TF only in its single model-wide primary topic from post-model binding and target evidence.';if(CONDITION_DOCUMENT_MODE&&topicCutoff&&!topicCutoff.dataset.conditionDefault){topicCutoff.dataset.conditionDefault='1';topicCutoff.value='0.1';if(topicPreset)topicPreset.value='custom'}conditionTopicMetric.disabled=overall;tfSelect.disabled=overall;pathwayScoreMethod.disabled=overall;pathwayDeOnly.disabled=!paired;byId('networkCorrectDirectionOnly').disabled=!paired;if(!pathwayDeOnly.dataset.bound){pathwayDeOnly.dataset.bound='1';pathwayDeOnly.addEventListener('change',()=>{pathwaySelect.value='';PAGE_INDEX.path=0;drawPathways();sendState(false);if(networkOpen){updateNetworkHeading();drawNetwork()}})}const directionOnly=byId('networkCorrectDirectionOnly');if(!directionOnly.dataset.bound){directionOnly.dataset.bound='1';directionOnly.addEventListener('change',()=>{if(networkOpen)drawNetwork()})}ensureMdsConditionVisible(cond1Select.value);ensureMdsConditionVisible(cond2Select.value);updateProbabilityPanelTitles();drawMds();drawActivity();drawButterfly();drawTfButterfly();drawPathways();syncReportMode();if(networkOpen){updateNetworkHeading();drawNetwork()}sendState(false)}
...
if(byId('thetaAggregation'))byId('thetaAggregation').addEventListener('change',()=>{pathwaySelect.value='';PAGE_INDEX.topic=0;PAGE_INDEX.path=0;refresh()});
...
function tfPassesTopicInCondition(condition,tf,topic,cutoff,primary){
  return tfTheta(condition,tf,topic)>=cutoff&&(!primary||tfOverallPrimaryTopic(tf)===topic)
}
...
function rowTfPassesSelectedTopic(row,tf){
  ...
  return passesCutoff&&(!primary||tfOverallPrimaryTopic(tf)===topic)
}
...
.m3cr_condition_report_html <- function(title,
  ...
  tf_keep <- intersect(c(
    ...
    "topic", "topic_num", "theta", "primary_topic_num"
  ), names(tf_topic))
  packed <- lapply(list(
...
function syncChild(msg){const opts=msg.options||{},state=msg.state||{};childControls.forEach(id=>{const sel=document.getElementById(id+'Select'),rows=opts[id==='cond1'||id==='cond2'?'conditions':id==='topic'?'topics':id==='tf'?'tfs':'pathways']||[],alphabetic=id!=='topic';let use=alphabetic?rows.slice().sort((a,b)=>String(a.label).localeCompare(String(b.label),undefined,{sensitivity:'base'})):rows;if(id==='cond1')use=[{value:'',label:'Overall pathways'}].concat(use.filter(x=>x.value));if(id==='cond2')use=[{value:'',label:'None'}].concat(use.filter(x=>x.value));if(id==='tf')use=[{value:'',label:'All'}].concat(use.filter(x=>x.value));if(id==='pathway')use=[{value:'',label:'None'}].concat(use.filter(x=>x.value));const requested=id==='pathway'&&pendingPathway!==null&&use.some(x=>String(x.value)===String(pendingPathway))?pendingPathway:state[id];fill(sel,use,requested);if(id==='pathway'&&pendingPathway!==null&&String(state[id]||'')===String(pendingPathway))pendingPathway=null;sel.disabled=!use.length||(id==='cond2'&&!state.cond1)||(id==='tf'&&!state.cond1)});if(state.activeConditionSide)setActiveConditionSide(state.activeConditionSide,false);const conditions=opts.conditions||[];Object.entries(state.colors||{}).forEach(([k,v])=>{if(!conditionColors[k]&&/^#[0-9a-f]{6}$/i.test(String(v)))conditionColors[k]=v});buildPalette(conditions);document.getElementById('cond2Select').disabled=!state.cond1;document.getElementById('tfSelect').disabled=!state.cond1;cond1Color.disabled=!state.cond1;cond2Color.disabled=!state.cond2;conditionTopicMetric.disabled=!state.cond1}
