...
function mdsLabelCandidates(q,bounds){const out=[],w=q.w,h=q.h,x=q.pointX,y=q.pointY;[14,28,44,64,88,116].forEach(gap=>[[x+gap,y-h/2,'right'],[x-w-gap,y-h/2,'left'],[x+gap,y-h-gap,'top-right'],[x-w-gap,y-h-gap,'top-left'],[x+gap,y+gap,'bottom-right'],[x-w-gap,y+gap,'bottom-left'],[x-w/2,y-h-gap,'top'],[x-w/2,y+gap,'bottom']].forEach((d,i)=>out.push({x:d[0],y:d[1],w,h,location:d[2],rank:i+gap/14})));[18,48,84].forEach(gap=>[-140,-112,-84,-56,-28,0,28,56,84,112,140].forEach(dy=>{out.push({x:x+gap,y:y+dy-h/2,w,h,location:'right-track',rank:12+gap/14+Math.abs(dy)/28});out.push({x:x-w-gap,y:y+dy-h/2,w,h,location:'left-track',rank:12+gap/14+Math.abs(dy)/28})}));for(let gy=bounds.y;gy<=bounds.y+bounds.h-h;gy+=28){out.push({x:bounds.x,y:gy,w,h,location:'left-margin',rank:35});out.push({x:bounds.x+bounds.w-w,y:gy,w,h,location:'right-margin',rank:35})}return out.map(d=>mdsClampLabelCandidate(d,bounds))}
...
function mdsLayoutLabels(items,W,H,pad,compact=false){const bounds=compact?{x:90,y:90,w:W-180,h:H-180}:{x:pad+6,y:pad+5,w:W-2*pad-12,h:H-2*pad-10},points=items.map(q=>({x:q.x,y:q.y,r:24,id:q.id})),candidatesFor=q=>{const source=mdsLabelCandidates(q,bounds),own={x:q.pointX-28,y:q.pointY-28,w:56,h:56},all=q.compact?source.filter(d=>mdsRectOverlap(d,own)===0):source;if(!Number.isFinite(q.maxLeader))return all;const near=all.filter(d=>{const e=mdsCandidateLeader(q,d);return Math.hypot(e.x2-e.x1,e.y2-e.y1)<=q.maxLeader});return near.length?near:all};items.forEach(q=>{q.compact=compact;q.fontSize=compact?17:18;q.w=Math.min(bounds.w,mdsTextWidth(q.text,q.fontSize));q.h=compact?27:36;q.maxLeader=compact?100:Infinity;q.nearest=Math.min(...points.filter(p=>p.id!==q.id).map(p=>Math.hypot(q.x-p.x,q.y-p.y)),Infinity)});const ordered=items.slice().sort((a,b)=>a.nearest-b.nearest||a.id.localeCompare(b.id)),placed=[];ordered.forEach(q=>{const candidates=candidatesFor(q),best=candidates.reduce((winner,d)=>mdsLabelPenalty(d,q,placed,points,bounds)<mdsLabelPenalty(winner,q,placed,points,bounds)?d:winner,candidates[0]);Object.assign(q,best);placed.push(q)});for(let pass=0;pass<10;pass++)ordered.forEach(q=>{const others=ordered.filter(d=>d!==q),candidates=candidatesFor(q),best=candidates.reduce((winner,d)=>mdsLabelPenalty(d,q,others,points,bounds)<mdsLabelPenalty(winner,q,others,points,bounds)?d:winner,candidates[0]);Object.assign(q,best)});const final=[];ordered.forEach(q=>{const candidates=candidatesFor(q),best=candidates.reduce((winner,d)=>mdsFinalLabelPenalty(d,q,final,points,bounds)<mdsFinalLabelPenalty(winner,q,final,points,bounds)?d:winner,candidates[0]);Object.assign(q,best);final.push(q)});for(let pass=0;pass<4;pass++)ordered.forEach(q=>{const others=ordered.filter(d=>d!==q),candidates=candidatesFor(q),best=candidates.reduce((winner,d)=>mdsFinalLabelPenalty(d,q,others,points,bounds)<mdsFinalLabelPenalty(winner,q,others,points,bounds)?d:winner,candidates[0]);Object.assign(q,best)});return items}
...
function colorScale(v,limit,palette='condition',single=false,role='node'){if(palette==='condition'){const t=clamp(v/Math.max(limit,1e-9),single?0:-1,1);if(single){const colors=role==='TF'?['#F2D7DF','#9F1748']:role==='Gene'?['#D7EBEE','#056A78']:['#D7DDDA',mix(conditionColor(1),'#111827',.18)];return mix(colors[0],colors[1],t)}const p=[mix(conditionColor(2),'#111827',.14),'#D4DAD7',mix(conditionColor(1),'#111827',.14)];return t<0?mix(p[1],p[0],-t):mix(p[1],p[2],t)}const diverging={rdbu:['#2166AC','#E0E3E1','#B2182B'],prgn:['#762A83','#E0E3E1','#1B7837'],brbg:['#8C510A','#E0E3E1','#01665E']};let p=diverging[palette];if(!p){const q=NETWORK_COLOR_PRESETS[palette]||NETWORK_COLOR_PRESETS.default;p=[q[1],'#E0E3E1',q[0]]}if(single){const t=clamp(v/Math.max(limit,1e-9),0,1);return mix('#D7DDDA',p[2],t)}const t=clamp(v/Math.max(limit,1e-9),-1,1);return t<0?mix(p[1],p[0],-t):mix(p[1],p[2],t)}function mix(a,b,t){const rgb=x=>{const s=String(x||'').trim();if(/^#[0-9a-f]{6}$/i.test(s)){const q=parseInt(s.slice(1),16);return[(q>>16)&255,(q>>8)&255,q&255]}const m=s.match(/[\d.]+/g);return m&&m.length>=3?m.slice(0,3).map(Number):[127,127,127]},x=rgb(a),y=rgb(b);return'rgb('+x.map((v,i)=>Math.round(v+(y[i]-v)*t)).join(',')+')'}
...
function edgeRows(cond){return EDGE_BY_COND.get(String(cond))||[]}function edgeRowsForTf(cond,tf){return(EDGE_BY_COND_TF.get(String(cond))||new Map()).get(tfKey(tf))||[]}function auditGeneExpr(cond,gene,fallback){const v=GENE_EXPR_INDEX.get(String(cond)+'\t'+geneKey(gene));return Number.isFinite(v)?v:LOADED_EXPRESSION_CONDITIONS.has(String(cond))?NaN:fallback}function tfExpression(cond,tf,fallback=NaN){const v=TF_EXPR_INDEX.get(String(cond)+'\t'+tfKey(tf));return Number.isFinite(v)?v:LOADED_EXPRESSION_CONDITIONS.has(String(cond))?NaN:fallback}function decorateLinkDirection(r){const cfg=directionConfig(),paired=!!cond2Select.value,geneReady=paired&&Number.isFinite(r.gexprA)&&Number.isFinite(r.gexprB),tfA=r.exprA??r.tfExprA,tfB=r.exprB??r.tfExprB,tfReady=paired&&Number.isFinite(tfA)&&Number.isFinite(tfB),geneLfc=geneReady?Math.log2((Math.max(0,r.gexprA)+cfg.pseudocount)/(Math.max(0,r.gexprB)+cfg.pseudocount)):NaN,tfLfc=tfReady?Math.log2((Math.max(0,tfA)+cfg.pseudocount)/(Math.max(0,tfB)+cfg.pseudocount)):NaN,fpChange=paired?(cfg.fpMode==='log2fc'?Math.log2((Math.max(0,r.a)+cfg.pseudocount)/(Math.max(0,r.b)+cfg.pseudocount)):r.a-r.b):r.a,targetPass=paired&&Number.isFinite(geneLfc)&&Math.abs(geneLfc)>=cfg.geneCut,fpPass=paired&&Math.abs(fpChange)>=cfg.fpCut,sameDirection=targetPass&&fpPass&&Math.sign(geneLfc)===Math.sign(fpChange),tfOpposing=paired&&Number.isFinite(tfLfc)&&Math.abs(tfLfc)>=cfg.tfCut&&Math.sign(tfLfc)===-Math.sign(geneLfc);return Object.assign(r,{delta:paired?r.a-r.b:r.a,geneLfc,tfLfc,fpChange,correctDirection:paired&&sameDirection&&!tfOpposing,tfOpposing})}function edgePairIndex(){const c1=cond1Select.value,c2=cond2Select.value,key=[DATA_VERSION,c1,c2].join('\t');if(EDGE_PAIR_CACHE.key===key)return EDGE_PAIR_CACHE;const m=new Map();edgeRows(c1).forEach(d=>m.set(tfKey(d.tf)+'\t'+d.gene_key,{tf:d.tf,tfu:tfKey(d.tf),gene:String(d.gene_key),a:num(d.fp_sum),b:0,exprA:tfExpression(c1,d.tf,num(d.tf_expr,NaN)),exprB:NaN,gexprA:num(d.gene_expr,NaN),gexprB:NaN,nA:num(d.n_peaks),nB:0}));if(c2)edgeRows(c2).forEach(d=>{const k=tfKey(d.tf)+'\t'+d.gene_key,r=m.get(k)||{tf:d.tf,tfu:tfKey(d.tf),gene:String(d.gene_key),a:0,b:0,exprA:tfExpression(c1,d.tf,NaN),exprB:NaN,gexprA:NaN,gexprB:NaN,nA:0,nB:0};r.b=num(d.fp_sum);r.exprB=tfExpression(c2,d.tf,num(d.tf_expr,NaN));r.gexprB=num(d.gene_expr,NaN);r.nB=num(d.n_peaks);m.set(k,r)});const rows=Array.from(m.values()).map(r=>decorateLinkDirection(Object.assign(r,{exprA:tfExpression(c1,r.tfu,r.exprA),exprB:c2?tfExpression(c2,r.tfu,r.exprB):NaN,gexprA:auditGeneExpr(c1,r.gene,r.gexprA),gexprB:c2?auditGeneExpr(c2,r.gene,r.gexprB):NaN}))),byGene=new Map(),byTf=new Map();rows.forEach(r=>{const geneRows=byGene.get(r.gene)||[],tfRows=byTf.get(r.tfu)||[];geneRows.push(r);tfRows.push(r);byGene.set(r.gene,geneRows);byTf.set(r.tfu,tfRows)});EDGE_PAIR_CACHE={key,rows,byGene,byTf};return EDGE_PAIR_CACHE}function edgePair(){return edgePairIndex().rows}
...
function drawMds(){
  ...
  const W=760,H=760,pad=72,pointInset=38,rows=mdsVisibleRows(),
    crowded=rows.length>12,plotLeft=crowded?90:pad,
    plotRight=crowded?670:W-pad,plotTop=crowded?90:pad,
    plotBottom=crowded?670:H-pad,dataInset=crowded?42:pointInset;
  ...
  mdsLayoutLabels(labels,W,H,crowded?54:pad,crowded).forEach(q=>{
