...
function drawMds(){const g=byId('mdsLayer');g.replaceChildren();const W=760,H=760,pad=72,pointInset=38,rows=GROUP_MDS,sx=scale(rows.map(d=>num(d.MDS1)),pad+pointInset,W-pad-pointInset),sy=scale(rows.map(d=>num(d.MDS2)),H-pad-pointInset,pad+pointInset),selected=new Set([cond1Select.value,cond2Select.value].filter(Boolean)),shorten=byId('shortConditionNames')&&byId('shortConditionNames').checked,labelMap=shorten?mdsShortLabelMap():mdsFullLabelMap(),labels=[];g.appendChild(el('line',{x1:pad,y1:H-pad,x2:W-pad,y2:H-pad,stroke:'#111','stroke-width':1.5}));g.appendChild(el('line',{x1:pad,y1:pad,x2:pad,y2:H-pad,stroke:'#111','stroke-width':1.5}));const xTitle=el('text',{x:W/2,y:H-18,'text-anchor':'middle','font-size':15,'font-weight':700,fill:'#303834'});xTitle.textContent='MDS1';g.appendChild(xTitle);const yTitle=el('text',{x:20,y:H/2,'text-anchor':'middle','font-size':15,'font-weight':700,fill:'#303834',transform:'rotate(-90 20 '+(H/2)+')'});yTitle.textContent='MDS2';g.appendChild(yTitle);rows.forEach(d=>{const id=String(d.comparison_label||d.group_label),x=sx(num(d.MDS1)),y=sy(num(d.MDS2)),sel=selected.has(id),c=conditionColorForId(id),raw=String(d.mds_label||d.display_label||id).replace(/_/g,' ').trim(),text=labelMap.get(raw)||raw,p=el('circle',{cx:x,cy:y,r:14,fill:c,stroke:'#fff','stroke-width':2.5,style:'cursor:pointer','data-condition-id':id});p.addEventListener('click',()=>selectMdsCondition(id));p.addEventListener('mousemove',ev=>tooltip('mdsTooltip',ev,(d.display_label||id)+'\ndocuments: '+num(d.n_docs)));p.addEventListener('mouseout',()=>hideTip('mdsTooltip'));g.appendChild(p);if(sel)g.appendChild(el('circle',{cx:x,cy:y,r:7,fill:'#111',stroke:'none','pointer-events':'none','data-selection-marker':'true'}));labels.push({d,id,pointX:x,pointY:y,x,y,c,p,text})});mdsLayoutLabels(labels,W,H,pad).forEach(q=>{const e=mdsLeaderEndpoints(q);g.appendChild(el('line',{x1:e.x1,y1:e.y1,x2:e.x2,y2:e.y2,stroke:'#59635e','stroke-width':1.25,opacity:.66}));const t=el('text',{x:q.x+5,y:q.y+25,'text-anchor':'start','font-size':18,'font-weight':700,fill:q.c,style:'cursor:pointer;paint-order:stroke;stroke:#fff;stroke-width:5px;stroke-linejoin:round'});t.textContent=q.text;t.addEventListener('click',()=>q.p.dispatchEvent(new MouseEvent('click')));t.addEventListener('mousemove',ev=>tooltip('mdsTooltip',ev,(q.d.display_label||q.id)+'\ndocuments: '+num(q.d.n_docs)));t.addEventListener('mouseout',()=>hideTip('mdsTooltip'));g.appendChild(t)});styleMdsPlot();byId('mdsStats').textContent=rows.length+' conditions'}function scale(vals,lo,hi){const x=vals.filter(Number.isFinite);if(!x.length)return()=>((lo+hi)/2);const a=Math.min(...x),b=Math.max(...x);return v=>a===b?(lo+hi)/2:lo+(v-a)/(b-a)*(hi-lo)}
...
function mdsPruneLeaderLines(){const nodes=Array.from(byId('mdsLayer').querySelectorAll('line')).filter(x=>x.getAttribute('stroke')==='#59635e'),candidates=nodes.map((node,i)=>{const segment={x1:num(node.getAttribute('x1')),y1:num(node.getAttribute('y1')),x2:num(node.getAttribute('x2')),y2:num(node.getAttribute('y2'))},d=GROUP_MDS[i]||{},id=String(d.comparison_label||d.group_label||i);return{node,segment,id,length:Math.hypot(segment.x2-segment.x1,segment.y2-segment.y1)}}),visible=new Set(mdsVisibleLeaders(candidates).map(d=>d.node));candidates.forEach(d=>{if(!visible.has(d.node))d.node.remove()})}
