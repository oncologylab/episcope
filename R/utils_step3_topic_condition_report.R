...
function indexPayload(){EDGE_BY_COND=new Map();PEAK_BY_COND_PAIR=new Map();GENE_EXPR_INDEX=new Map();PATH_ACTIVITY_INDEX=new Map();PAYLOAD.tf_gene.forEach(d=>{const c=String(d.condition_id),a=EDGE_BY_COND.get(c)||[];a.push(d);EDGE_BY_COND.set(c,a)});PAYLOAD.tf_peak_gene.forEach(d=>{const c=String(d.condition_id),cm=PEAK_BY_COND_PAIR.get(c)||new Map(),k=tfKey(d.tf)+'\t'+d.gene_key,a=cm.get(k)||[];a.push(d);cm.set(k,a);PEAK_BY_COND_PAIR.set(c,cm)});(PAYLOAD.pathway_gene_expression||[]).forEach(d=>GENE_EXPR_INDEX.set(String(d.condition_id)+'\t'+String(d.gene_key),num(d.gene_expr,NaN)));(PAYLOAD.pathway_activity||[]).forEach(d=>PATH_ACTIVITY_INDEX.set(Number(d.topic_num)+'\t'+pathKey(d.pathway_key)+'\t'+String(d.condition_id),d))}
...
function pathwayRows(){const topic=Number(topicSelect.value),c1=cond1Select.value,c2=cond2Select.value,condition1First=!c2||groupTopic(c1,topic)>=groupTopic(c2,topic);let rows=PATHWAYS.filter(d=>topicNum(d.topic_num||d.topic)===topic&&num(d.padj,1)<=.05).map(d=>{const key=pathKey(d.pathway_key||d.pathway_norm_key||d.pathway),a=pathwayActivity(topic,key,c1),b=c2?pathwayActivity(topic,key,c2):null,scoreA=pathwayScore(a),scoreB=c2?pathwayScore(b):NaN,genes=splitGenes(d.genes||d.overlap_genes),nA=num(a&&a.n_expression_genes),nB=c2?num(b&&b.n_expression_genes):0;return{key,pathway:String(d.pathway),scoreA,scoreB,delta:c2?scoreB-scoreA:scoreA,padj:num(d.padj,1),overlap:Math.max(nA,nB,num(d.overlap_hits||d.gene_in)),genesA:genes,genesB:genes,genes,activityA:a,activityB:b}}).filter(r=>Number.isFinite(r.scoreA)&&(!c2||Number.isFinite(r.scoreB))&&r.overlap>=3);const tf=tfSelect.value;if(tf){const ta=selectedTfTargets(c1,tf),tb=c2?selectedTfTargets(c2,tf):new Set();rows=rows.map(r=>{const tfA=r.genes.filter(g=>ta.has(g)),tfB=c2?r.genes.filter(g=>tb.has(g)):[],tfUnion=Array.from(new Set(tfA.concat(tfB)));return Object.assign(r,{tfA,tfB,tfUnion,tfCountA:tfA.length,tfCountB:tfB.length,tfCountUnion:tfUnion.length})}).filter(r=>r.tfCountUnion>0);if(!c2)return rows.sort((a,b)=>b.tfCountA-a.tfCountA||b.tfCountUnion-a.tfCountUnion||a.padj-b.padj||a.pathway.localeCompare(b.pathway));const c1Tf=rows.filter(d=>d.delta<=0).sort((a,b)=>b.tfCountA-a.tfCountA||b.tfCountUnion-a.tfCountUnion||a.delta-b.delta||a.padj-b.padj||a.pathway.localeCompare(b.pathway)),c2Tf=rows.filter(d=>d.delta>0).sort((a,b)=>b.tfCountB-a.tfCountB||b.tfCountUnion-a.tfCountUnion||b.delta-a.delta||a.padj-b.padj||a.pathway.localeCompare(b.pathway));return condition1First?c1Tf.concat(c2Tf):c2Tf.concat(c1Tf)}if(!c2)return rows.sort((a,b)=>b.scoreA-a.scoreA||a.padj-b.padj||a.pathway.localeCompare(b.pathway)).slice(0,PAGE_SIZE.path);const half=Math.floor(PAGE_SIZE.path/2),c1Rows=rows.filter(d=>d.delta<=0).sort((a,b)=>a.delta-b.delta||a.padj-b.padj||a.pathway.localeCompare(b.pathway)).slice(0,half),c2Rows=rows.filter(d=>d.delta>0).sort((a,b)=>b.delta-a.delta||a.padj-b.padj||a.pathway.localeCompare(b.pathway)).slice(0,PAGE_SIZE.path-half);return condition1First?c1Rows.concat(c2Rows):c2Rows.concat(c1Rows)}
...
function addPathLegend(g){const sig=el('text',{x:24,y:20,class:'pathLegend',fill:'#9A6700'});sig.textContent='Overall topic significance: * q<=0.05 | ** q<=0.01 | *** q<=0.001 | **** q<=0.0001';g.appendChild(sig);const size=el('text',{x:650,y:20,class:'pathLegend',fill:'#46524d'});size.textContent='Dot size = expression-matched genes';g.appendChild(size)}
function drawPathways(){PATH_ROWS=pathwayRows();updatePathwaySelect(PATH_ROWS);const g=byId('pathLayer');g.replaceChildren();const W=1120,H=900,LABEL_X=630,PLOT_L=680,R=54,PLOT_R=W-R,T=112,B=68,AXIS_Y=82,BOTTOM_AXIS_Y=H-B+8,rows=pageRows(PATH_ROWS,'path'),rowH=Math.min(22,(H-T-B)/Math.max(rows.length,1)),scoreLabel=pathwayScoreMethod.value==='rank_enrichment'?'ranked-expression enrichment z-score':'mean standardized-expression score',axisScoreLabel=pathwayScoreMethod.value==='rank_enrichment'?'RNA rank-enrichment delta':'mean-expression delta';addPathLegend(g);differentialConditionLabels(g,cond1Select.value,cond2Select.value,PLOT_L+(PLOT_R-PLOT_L)/4,PLOT_L+3*(PLOT_R-PLOT_L)/4,43);if(!rows.length){const t=el('text',{x:30,y:112,'font-size':17,'font-weight':700});t.textContent=tfSelect.value?'No significant overall topic pathways contain direct targets of the selected TF.':'No significant overall topic pathways have at least three expression-matched genes.';g.appendChild(t);byId('pathStats').textContent='0 pathways';return}const max=Math.max(...PATH_ROWS.map(r=>Math.abs(r.delta)),.05),maxLogp=Math.max(...PATH_ROWS.map(r=>-Math.log10(Math.max(r.padj,1e-300))),1),zero=(PLOT_L+PLOT_R)/2,half=(PLOT_R-PLOT_L)/2,sx=v=>zero+(v/max)*half,formatTick=v=>Math.abs(v)>=1?v.toFixed(1):v.toFixed(2),axisText=cond2Select.value?conditionLabel(cond1Select.value)+' <- '+axisScoreLabel+' -> '+conditionLabel(cond2Select.value):scoreLabel+': '+conditionLabel(cond1Select.value);rows.forEach((r,i)=>{const y=T+i*rowH;if(i%2===1)g.appendChild(el('rect',{x:8,y:y-1,width:W-16,height:rowH,fill:'#f7f8f6'}))});[-1,-.5,0,.5,1].forEach(frac=>{const value=frac*max,xx=sx(value),major=frac===0||Math.abs(frac)===1;g.appendChild(el('line',{x1:xx,y1:AXIS_Y,x2:xx,y2:BOTTOM_AXIS_Y,stroke:frac===0?'#4b5550':'#8a9690','stroke-width':frac===0?1.3:.9,'stroke-dasharray':frac===0?'5 4':'2 5',opacity:major?1:.3}));[AXIS_Y+16,BOTTOM_AXIS_Y+17].forEach(ty=>{const tick=el('text',{x:xx,y:ty,'text-anchor':'middle','font-size':11,'font-weight':700,fill:'#46524d',class:'pathAxisTickLabel'});tick.textContent=formatTick(value);g.appendChild(tick)})});[AXIS_Y,BOTTOM_AXIS_Y].forEach(y=>g.appendChild(el('line',{x1:PLOT_L,y1:y,x2:PLOT_R,y2:y,stroke:'#111','stroke-width':1.6})));[64,H-7].forEach((y,i)=>{const axis=el('text',{x:(PLOT_L+PLOT_R)/2,y,'text-anchor':'middle','font-size':i?13:12,'font-weight':700,fill:'#303834'});axis.textContent=axisText;g.appendChild(axis)});rows.forEach((r,i)=>{const y=T+i*rowH,yy=y+rowH/2,selected=r.key===pathwaySelect.value;if(selected)g.appendChild(el('rect',{x:8,y:y-1,width:W-16,height:rowH,rx:3,class:'pathRowSelected'}));const lab=wrapText(g,r.pathway,LABEL_X,yy,LABEL_X-16,pathwayLabelClass(r.pathway)),stars=pathwayStars(r.padj),star=el('text',{x:LABEL_X+7,y:yy+4,'text-anchor':'start','font-size':12,'font-weight':700,fill:'#9A6700',style:'cursor:pointer'}),x=sx(r.delta),rad=Math.min(12,3.2+Math.sqrt(Math.max(1,r.overlap))*.85),opacity=.5+.5*clamp(-Math.log10(Math.max(r.padj,1e-300))/maxLogp,0,1),dot=el('circle',{cx:x,cy:yy,r:rad,fill:cond2Select.value&&r.delta>0?conditionColor(2):conditionColor(1),opacity,stroke:selected?'#111':'#fff','stroke-width':selected?4:2,style:'cursor:pointer'}),countText=tfSelect.value?(cond2Select.value?r.tfCountA+'/'+r.tfCountB+'/'+r.tfCountUnion:String(r.tfCountA)):'',orderedA=orderedPathGenes(r.genes,cond1Select.value),orderedB=cond2Select.value?orderedPathGenes(r.genes,cond2Select.value):[],msg=r.pathway+'\nOverall topic-pathway q: '+r.padj.toExponential(2)+' '+stars+'\n'+conditionLabel(cond1Select.value)+' '+scoreLabel+': '+r.scoreA.toFixed(3)+(cond2Select.value?'\n'+conditionLabel(cond2Select.value)+' '+scoreLabel+': '+r.scoreB.toFixed(3)+'\n'+conditionLabel(cond2Select.value)+' - '+conditionLabel(cond1Select.value)+' difference: '+r.delta.toFixed(3):'')+'\nexpression-matched genes: '+r.overlap+'\n'+conditionLabel(cond1Select.value)+' genes by expression: '+orderedA.map(d=>d.gene+' ('+d.expr.toFixed(2)+')').join(', ')+(cond2Select.value?'\n'+conditionLabel(cond2Select.value)+' genes by expression: '+orderedB.map(d=>d.gene+' ('+d.expr.toFixed(2)+')').join(', '):'')+(tfSelect.value?'\nTF target counts ('+conditionLabel(cond1Select.value)+(cond2Select.value?' / '+conditionLabel(cond2Select.value)+' / union':'')+'): '+countText+'\n'+conditionLabel(cond1Select.value)+' TF targets: '+(r.tfA||[]).join(',')+(cond2Select.value?'\n'+conditionLabel(cond2Select.value)+' TF targets: '+(r.tfB||[]).join(', '):''):'');star.textContent=stars;const lineStep=9.2;lab.node.setAttribute('y',yy-((lab.lines-1)*lineStep)/2+3.5);[dot,lab.node,star].forEach(n=>{n.addEventListener('click',()=>{lastNetworkTrigger=n;pathwaySelect.value=r.key;sendState(false);openNetwork()});n.addEventListener('mousemove',ev=>tooltip('pathTooltip',ev,msg));n.addEventListener('mouseout',()=>hideTip('pathTooltip'))});g.appendChild(star);g.appendChild(dot);if(countText){const ct=el('text',{x:x+(x>PLOT_R-75?-rad-5:rad+5),y:yy+4,'text-anchor':x>PLOT_R-75?'end':'start','font-size':11,'font-weight':700,fill:'#111',style:'pointer-events:none;paint-order:stroke;stroke:#fff;stroke-width:3px'});ct.textContent=countText;g.appendChild(ct)}});const start=PAGE_INDEX.path*PAGE_SIZE.path+1;byId('pathStats').textContent=start+'-'+(start+rows.length-1)+' of '+PATH_ROWS.length+' significant overall topic pathways'+(tfSelect.value?' with direct '+tfLabel()+' targets; counts = condition 1 / condition 2 / union':'')}
...
function openNetwork(){networkOpen=true;document.body.classList.add('network-open');byId('networkPanel').style.display='flex';const path=currentPath(),title=path?path.pathway:'Selected pathway network',context=conditionLabel(cond1Select.value)+(cond2Select.value?' vs '+conditionLabel(cond2Select.value):'')+' | Topic '+topicSelect.value+(tfSelect.value?' | TF '+tfLabel():' | All TFs');byId('networkTitle').textContent=title;byId('networkTitle').title=title;byId('networkContext').textContent=context;byId('networkStats').textContent='Loading compressed network data...';networkState.view={x:0,y:0,k:1};const loads=[ensureSelectedConditionEdges(),loadNetworkPayload()];if(byId('networkMode').value==='tf_peak_gene')loads.push(ensureSelectedConditionPeaks());Promise.all(loads).then(()=>{if(networkOpen)drawNetwork()}).catch(e=>{byId('networkStats').textContent=e.message});byId('networkBack').focus()}function closeNetwork(){networkOpen=false;document.body.classList.remove('network-open');byId('networkPanel').style.display='none';if(lastNetworkTrigger&&typeof lastNetworkTrigger.focus==='function')lastNetworkTrigger.focus()}
...
function updateProbabilityPanelTitles(){const label=tfSelect.value?' - '+tfLabel():'';byId('conditionProbabilityTitle').textContent='Condition Probability'+label;byId('conditionProbabilityTitle').title=tfSelect.value?'Condition probabilities with selected TF: '+tfLabel():'Condition Probability';byId('tfConditionProbabilityTitle').textContent='TF Probability'+label;byId('tfConditionProbabilityTitle').title=tfSelect.value?'Condition::TF topic probabilities for '+tfLabel():'TF probabilities for the selected topic'}
...
.m3cr_condition_report_html <- function(title,
  ...
  html <- c(
    ...
    "<meta name=\"craftgrn-module3-report-schema\" content=\"7\"/>",
    ...
    "</div></section>",
    "<section class=\"pane\"><div class=\"bottomPair\"><div class=\"mini\"><div class=\"paneHead\"><h2 id=\"conditionProbabilityTitle\">Condition Probability</h2><div class=\"headTools\"><span class=\"help\" title=\"Mean condition theta by topic. Topic labels and bars are selectable.\">?</span><span id=\"butterflyStats\" class=\"meta\"></span><span class=\"pager\"><button id=\"topicPrev\" class=\"secondary\" type=\"button\" title=\"Previous topics\" aria-label=\"Previous topics\">&lt;</button><span id=\"topicPageStatus\" class=\"pageStatus\"></span><button id=\"topicNext\" class=\"secondary\" type=\"button\" title=\"Next topics\" aria-label=\"Next topics\">&gt;</button></span></div></div><div class=\"body\"><div id=\"butterflyTooltip\" class=\"tooltip\"></div><div class=\"fixedChart\"><svg id=\"butterflySvg\" viewBox=\"0 0 760 760\"><g id=\"butterflyLayer\"></g></svg></div></div></div><div class=\"mini\"><div class=\"paneHead\"><h2 id=\"tfConditionProbabilityTitle\">TF Probability</h2><div class=\"headTools\"><span class=\"help\" title=\"With TF set to All, this shows the ten largest positive and negative TF probability differences for the selected topic. With a TF selected, it preserves the Condition Probability topic order.\">?</span><span id=\"tfButterflyStats\" class=\"meta\"></span><span class=\"pager\"><button id=\"tfPrev\" class=\"secondary\" type=\"button\" title=\"Previous rows\" aria-label=\"Previous rows\">&lt;</button><span id=\"tfPageStatus\" class=\"pageStatus\"></span><button id=\"tfNext\" class=\"secondary\" type=\"button\" title=\"Next rows\" aria-label=\"Next rows\">&gt;</button></span></div></div><div class=\"body\"><div id=\"tfButterflyTooltip\" class=\"tooltip\"></div><div class=\"fixedChart\"><svg id=\"tfButterflySvg\" viewBox=\"0 0 760 760\"><g id=\"tfButterflyLayer\"></g></svg></div></div></div></div></section></div>",
    "<section class=\"pane\"><div class=\"paneHead\"><h2>Pathways</h2><div class=\"contextWrap\"><label class=\"pathScoreControl\">Score <select id=\"pathwayScoreMethod\"><option value=\"rank_enrichment\" selected>Ranked expression</option><option value=\"mean_gene_zscore\">Mean standardized expression</option></select></label><span id=\"pathStats\" class=\"meta\"></span><span class=\"pager\"><button id=\"pathPrev\" class=\"secondary\" type=\"button\" title=\"Previous pathways\" aria-label=\"Previous pathways\">&lt;</button><span id=\"pathPageStatus\" class=\"pageStatus\"></span><button id=\"pathNext\" class=\"secondary\" type=\"button\" title=\"Next pathways\" aria-label=\"Next pathways\">&gt;</button></span><div id=\"selectedContext\" class=\"contextBadge\"></div><span class=\"help\" title=\"Overall topic pathways with adjusted p <= 0.05. Each condition score ranks the fixed overlap genes against its complete available RNA gene universe. Position shows the condition-score difference; dot size is the number of expression-matched genes; asterisks report overall topic-pathway significance.\">?</span></div></div><div class=\"body\"><div id=\"pathTooltip\" class=\"tooltip\"></div><div class=\"fixedChart\"><svg id=\"pathSvg\" viewBox=\"0 0 1120 900\"><g id=\"pathLayer\"></g></svg></div>",
...
function reportOptions(){return methodRows().slice().sort((a,b)=>Number(a.trained_k)-Number(b.trained_k)||(a.topic_space==='raw'?-1:1)).map(r=>({value:r.report_key,label:r.report_label,topic_space:r.topic_space,trained_k:r.trained_k}))}function defaultReportOption(options){const d=REPORT_STATE.defaults||{},space=String(d.topic_space||'combined'),trained=Number(d.trained_k||30);return options.find(x=>x.topic_space===space&&Number(x.trained_k)===trained)||options.find(x=>x.topic_space==='combined'&&Number(x.trained_k)===30)||options[0]}function initRunControls(){fill(methodSelect,methods().map(x=>({value:x,label:x})),methods()[0]);const options=reportOptions();fill(kSelect,options,defaultReportOption(options)?.value);load()}
