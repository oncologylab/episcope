...
.m3tb_write_condition_mds_svg <- function(group_mds, out_svg) {
  ...
  p <- ggplot2::ggplot(dt, ggplot2::aes(MDS1, MDS2)) +
    ...
    ggplot2::geom_text(
      ...
      fontface = "bold",
      size = if (is_comparison) 4.7 else 5.2,
      show.legend = FALSE
    ) +
...
.m3tb_condition_report_html <- function(title, group_mds, group_topic, pathways, out_html, mds_svg_src = NULL) {
  ...
  mds_img <- if (!is.null(mds_svg_src) && nzchar(mds_svg_src)) {
    paste0("<img id=\"mdsImage\" class=\"mdsImage\" src=\"", .m3tb_html_escape(mds_svg_src), "\" alt=\"Condition MDS\"/><div id=\"mdsHotspotLayer\" class=\"mdsHotspotLayer\"></div><div class=\"tooltip\" id=\"mdsTooltip\"></div><svg id=\"mdsSvg\" viewBox=\"0 0 980 680\" style=\"display:none\"><g id=\"mdsLayer\"></g></svg>")
  } else {
