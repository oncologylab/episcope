...
.m3tb_topic_report_html <- function(title, topic_mds, waterfall, pathways, out_html) {
  ...
  html <- c(
    ...
    "function init(){TOPICS.sort((a,b)=>topicNum(a.topic)-topicNum(b.topic)).forEach(d=>{const o=document.createElement('option');o.value=d.topic;o.textContent='Topic '+topicNum(d.topic);topicSelect.appendChild(o)});topicSelect.addEventListener('change',draw);paletteSelect.addEventListener('change',draw);document.getElementById('exportSvgButton').addEventListener('click',()=>{const b=new Blob([document.documentElement.outerHTML],{type:'text/html'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=document.title.replace(/[^A-Za-z0-9_.-]+/g,'_')+'.html';a.click();URL.revokeObjectURL(a.href)});draw()}",
...
.m3tb_write_condition_mds_svg <- function(group_mds, out_svg) {
  ...
  grDevices::svg(out_svg, width = if (is_comparison) 9.8 else 7.2, height = 6.2, bg = "white")
  on.exit(grDevices::dev.off(), add = TRUE)
  print(p)
  invisible(TRUE)
}
...
.m3tb_condition_report_html <- function(title, group_mds, group_topic, pathways, out_html, mds_svg_src = NULL) {
  ...
  html <- c(
    ...
    "function drawMds(){mdsLayer.replaceChildren();const w=980,h=680,left=74,right=34,top=42,bottom=56,sel=groupSelect.value,dense=GROUP_MDS.length>12,labelSize=svgFontSize(mdsSvg,dense?14:16),smallSize=svgFontSize(mdsSvg,12),sx=sc(GROUP_MDS.map(d=>+d.MDS1),left+78,w-right-78),sy=sc(GROUP_MDS.map(d=>+d.MDS2),h-bottom-28,top+28),ticks=[-0.4,-0.2,0,0.2,0.4];mdsLayer.appendChild(el('line',{x1:left,y1:h-bottom,x2:w-right,y2:h-bottom,stroke:'#111','stroke-width':1.5}));mdsLayer.appendChild(el('line',{x1:left,y1:top,x2:left,y2:h-bottom,stroke:'#111','stroke-width':1.5}));ticks.forEach(t=>{const xx=sx(t),yy=sy(t);if(xx>=left&&xx<=w-right){mdsLayer.appendChild(el('line',{x1:xx,y1:h-bottom,x2:xx,y2:h-bottom+6,stroke:'#111','stroke-width':1}));mdsLayer.appendChild(el('text',{x:xx,y:h-bottom+24,'text-anchor':'middle','font-size':smallSize,'font-weight':700,fill:'#111'})).textContent=String(t)}if(yy>=top&&yy<=h-bottom){mdsLayer.appendChild(el('line',{x1:left-6,y1:yy,x2:left,y2:yy,stroke:'#111','stroke-width':1}));mdsLayer.appendChild(el('text',{x:left-12,y:yy+4,'text-anchor':'end','font-size':smallSize,'font-weight':700,fill:'#111'})).textContent=String(t)}});mdsLayer.appendChild(el('text',{x:(left+w-right)/2,y:h-12,'text-anchor':'middle','font-size':labelSize,'font-weight':700,fill:'#111'})).textContent='MDS1';const ylab=el('text',{x:20,y:(top+h-bottom)/2,'text-anchor':'middle','font-size':labelSize,'font-weight':700,fill:'#111',transform:'rotate(-90 20 '+((top+h-bottom)/2)+')'});ylab.textContent='MDS2';mdsLayer.appendChild(ylab);const pts=GROUP_MDS.map((d,i)=>({d,i,x:sx(+d.MDS1),y:sy(+d.MDS2),r:d.comparison_label===sel?13:10,label:String(d.mds_label||d.display_label||d.comparison_label)}));const anchors=pts.map(p=>{const side=p.x<(left+w-right)/2?'left':'right',tx=side==='left'?left+112:w-right-112;return{p,side,x:tx,y:p.y,w:Math.max(56,Math.min(132,p.label.length*7.2+14)),h:20}});['left','right'].forEach(side=>{const a=anchors.filter(v=>v.side===side).sort((u,v)=>u.p.y-v.p.y),minY=top+22,maxY=h-bottom-22,n=a.length;if(!n)return;const step=(maxY-minY)/Math.max(1,n-1);a.forEach((v,i)=>{v.y=n===1?(minY+maxY)/2:minY+i*step})});function pick(id){selectGroup(id)}pts.forEach(p=>{const hit=el('circle',{cx:p.x,cy:p.y,r:24,fill:'transparent',style:'cursor:pointer'});hit.addEventListener('click',()=>pick(p.d.comparison_label));mdsLayer.appendChild(hit);const isSel=p.d.comparison_label===sel,c=el('circle',{cx:p.x,cy:p.y,r:p.r,fill:groupColor(p.d,p.i),opacity:isSel?.96:.84,stroke:isSel?'#111':'#fff','stroke-width':isSel?4:2,style:'cursor:pointer'});c.addEventListener('click',()=>pick(p.d.comparison_label));c.addEventListener('mousemove',ev=>tooltip(mdsTooltip,ev,String(p.d.display_label||p.d.comparison_label)+'\\ndocs: '+p.d.n_docs));c.addEventListener('mouseout',()=>mdsTooltip.style.display='none');mdsLayer.appendChild(c)});anchors.forEach(a=>{const p=a.p,dx=a.x-p.x,dy=a.y-p.y,dist=Math.max(1,Math.hypot(dx,dy)),x1=p.x+dx/dist*(p.r+2),y1=p.y+dy/dist*(p.r+2),line=el('line',{x1:x1,y1:y1,x2:a.x,y2:a.y,class:'mdsLeader'});line.addEventListener('click',()=>pick(p.d.comparison_label));mdsLayer.appendChild(line);const lab=String(a.p.label).length>18?String(a.p.label).slice(0,17)+'.':String(a.p.label),t=el('text',{x:a.x,y:a.y+5,'text-anchor':'middle','font-size':labelSize,'font-weight':700,fill:groupColor(p.d,p.i),'paint-order':'stroke',stroke:'#fff','stroke-width':4,'stroke-linejoin':'round',style:'cursor:pointer'});t.textContent=lab;t.addEventListener('click',()=>pick(p.d.comparison_label));t.addEventListener('mousemove',ev=>tooltip(mdsTooltip,ev,String(p.d.display_label||p.d.comparison_label)+'\\ndocs: '+p.d.n_docs));t.addEventListener('mouseout',()=>mdsTooltip.style.display='none');mdsLayer.appendChild(t)});document.getElementById('mdsStats').textContent=GROUP_MDS.length+' groups'}",
...
.m3tb_write_combined_report_index <- function(out_file, reports, title) {
  ...
  html <- c(
    "<!doctype html><html><head><meta charset=\"utf-8\"/>",
    paste0("<title>", .m3tb_html_escape(title), "</title>"),
    "<style>html,body{width:100%;height:100%;overflow:hidden}body{margin:0;background:#f7f7f5;color:#111;font-family:Arial,Helvetica,sans-serif}.top{height:42px;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:6px 12px;border-bottom:1px solid #d6d6d0;background:#fff;box-sizing:border-box}h1{font-size:18px;line-height:1;margin:0;font-weight:700}.controls{display:flex;gap:8px;align-items:center;font-weight:700;white-space:nowrap}select{font:700 12px Arial,Helvetica,sans-serif;border:1px solid #aaa;background:#fff;color:#111;border-radius:3px;padding:4px 7px}iframe{display:block;width:100vw;height:calc(100vh - 42px);border:0;background:#fff}</style></head><body>",
    "<div class=\"top\">",
    paste0("<h1>", .m3tb_html_escape(title), "</h1>"),
